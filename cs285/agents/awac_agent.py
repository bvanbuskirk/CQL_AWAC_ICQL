...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        # may need to check dimensionality of inputs obs and action
        if not isinstance(action, torch.LongTensor):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        next_on_no = ptu.from_numpy(next_ob_no)
        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
              v_dim = ob_no.size(dim=0)
              tensor_ac = torch.tensor([i] * v_dim).to(ptu.device)
              val = self.get_qvals(self.exploitation_critic, ob_no, tensor_ac.type(torch.long))
              vals.append(val)
        else:
            
            for _ in range(n_actions):
                pass
        v_pi = torch.vstack(vals)
        v_pi = torch.mean(v_pi, dim=0)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            norm_bonus = normalize(expl_bonus, expl_bonus.mean(), self.running_rnd_rew_std)
            ...
            mixed_reward = explore_weight * norm_bonus + exploit_weight * re_n
